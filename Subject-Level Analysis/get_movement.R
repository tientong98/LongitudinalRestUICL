...
for (i in 1:length(subject)) {
  confound1 <- read.table(file = paste0("/Shared/oleary/functional/UICL/BIDS/derivatives/fmriprep/mid/fmriprep/sub-",subject[i],"/ses-",session[i],"/func/sub-",subject[i],"_ses-",session[i],"_task-mid_run-1_bold_confounds.tsv"), header = T, stringsAsFactors = F) %>% select(X, Y, Z, RotX, RotY, RotZ)
  confound2 <- read.table(file = paste0("/Shared/oleary/functional/UICL/BIDS/derivatives/fmriprep/mid/fmriprep/sub-",subject[i],"/ses-",session[i],"/func/sub-",subject[i],"_ses-",session[i],"_task-mid_run-2_bold_confounds.tsv"), header = T, stringsAsFactors = F) %>% select(X, Y, Z, RotX, RotY, RotZ)
  confound3 <- read.table(file = paste0("/Shared/oleary/functional/UICL/BIDS/derivatives/fmriprep/mid/fmriprep/sub-",subject[i],"/ses-",session[i],"/func/sub-",subject[i],"_ses-",session[i],"_task-mid_run-3_bold_confounds.tsv"), header = T, stringsAsFactors = F) %>% select(X, Y, Z, RotX, RotY, RotZ)
  cal <- function(dataframe) {
    for (j in c('dX', 'dY', 'dZ', 'dRotX', 'dRotY', 'dRotZ', 'excess')) {dataframe[j] <- NA}
    for (k in 2:nrow(dataframe)) {dataframe[k, "dX"] <- abs(dataframe[k, "X"] - dataframe[k-1, "X"])}
    for (k in 2:nrow(dataframe)) {dataframe[k, "dY"] <- abs(dataframe[k, "Y"] - dataframe[k-1, "Y"])}
    for (k in 2:nrow(dataframe)) {dataframe[k, "dZ"] <- abs(dataframe[k, "Z"] - dataframe[k-1, "Z"])}
    for (k in 2:nrow(dataframe)) {dataframe[k, "dRotX"] <- abs(dataframe[k, "RotX"] - dataframe[k-1, "RotX"])}
    for (k in 2:nrow(dataframe)) {dataframe[k, "dRotY"] <- abs(dataframe[k, "RotY"] - dataframe[k-1, "RotY"])}
    for (k in 2:nrow(dataframe)) {dataframe[k, "dRotZ"] <- abs(dataframe[k, "RotZ"] - dataframe[k-1, "RotZ"])}
    for (k in 2:nrow(dataframe)) {ifelse(dataframe[k, "dX"] > 3 | dataframe[k, "dY"] > 3 | dataframe[k, "dZ"] > 3 | 
                                         dataframe[k, "dRotX"] > 3 | dataframe[k, "dRotY"] > 3 | dataframe[k, "dRotZ"] > 3, 
                                         dataframe[k, "excess"] <- 1, dataframe[k, "excess"] <- 0)}
    return(dataframe)}
  confound1 <- cal(confound1)
  confound2 <- cal(confound2)
  confound3 <- cal(confound3)
  excess1 <- sum(confound1$excess, na.rm = T)
  excess2 <- sum(confound2$excess, na.rm = T)
  excess3 <- sum(confound3$excess, na.rm = T)
  excess.sum <- sum(excess1, excess2, excess3)
  id <- subject[i]
  df <- data.frame(id, excess1, excess2, excess3, excess.sum)
  write.table(df, file = 
                paste0("/Shared/oleary/functional/UICL/BIDS/derivatives/fmriprep/mid/fmriprep/excess_mid.txt"), 
              sep = '\t', row.names = F, col.names =! file.exists("/Shared/oleary/functional/UICL/BIDS/derivatives/fmriprep/mid/fmriprep/excess_mid.txt"), quote = F, append = T)
  }
...
for (i in 1:length(subject)) {
  confound1 <- read.table(file = paste0("/Shared/oleary/functional/UICL/BIDS/derivatives/fmriprep/stopsignal/fmriprep/sub-",subject[i],"/ses-",session[i],"/func/sub-",subject[i],"_ses-",session[i],"_task-stopsignal_run-1_bold_confounds.tsv"), header = T, stringsAsFactors = F) %>% select(X, Y, Z, RotX, RotY, RotZ)
  confound2 <- read.table(file = paste0("/Shared/oleary/functional/UICL/BIDS/derivatives/fmriprep/stopsignal/fmriprep/sub-",subject[i],"/ses-",session[i],"/func/sub-",subject[i],"_ses-",session[i],"_task-stopsignal_run-2_bold_confounds.tsv"), header = T, stringsAsFactors = F) %>% select(X, Y, Z, RotX, RotY, RotZ)
  cal <- function(dataframe) {
    for (j in c('dX', 'dY', 'dZ', 'dRotX', 'dRotY', 'dRotZ', 'excess')) {dataframe[j] <- NA}
    for (k in 2:nrow(dataframe)) {dataframe[k, "dX"] <- abs(dataframe[k, "X"] - dataframe[k-1, "X"])}
    for (k in 2:nrow(dataframe)) {dataframe[k, "dY"] <- abs(dataframe[k, "Y"] - dataframe[k-1, "Y"])}
  ...
  excess1 <- sum(confound1$excess, na.rm = T)
  excess2 <- sum(confound2$excess, na.rm = T)
  excess.sum <- sum(excess1, excess2)
  id <- subject[i]
  df <- data.frame(id, excess1, excess2, excess.sum)
  write.table(df, file = 
                paste0("/Shared/oleary/functional/UICL/BIDS/derivatives/fmriprep/stopsignal/fmriprep/excess_sst.txt"), 
              sep = '\t', row.names = F, col.names =! file.exists("/Shared/oleary/functional/UICL/BIDS/derivatives/fmriprep/stopsignal/fmriprep/excess_sst.txt"), quote = F, append = T)
}
...
for (i in 1:length(subject)) {
  ...
  cal <- function(dataframe) {
    for (j in c('dX', 'dY', 'dZ', 'dRotX', 'dRotY', 'dRotZ', 'excess')) {dataframe[j] <- NA}
    for (k in 2:nrow(dataframe)) {dataframe[k, "dX"] <- abs(dataframe[k, "trans_x"] - dataframe[k-1, "trans_x"])}
    for (k in 2:nrow(dataframe)) {dataframe[k, "dY"] <- abs(dataframe[k, "trans_y"] - dataframe[k-1, "trans_y"])}
    for (k in 2:nrow(dataframe)) {dataframe[k, "dZ"] <- abs(dataframe[k, "trans_z"] - dataframe[k-1, "trans_z"])}
    for (k in 2:nrow(dataframe)) {dataframe[k, "dRotX"] <- abs(dataframe[k, "rot_x"] - dataframe[k-1, "rot_x"])}
    for (k in 2:nrow(dataframe)) {dataframe[k, "dRotY"] <- abs(dataframe[k, "rot_y"] - dataframe[k-1, "rot_y"])}
    for (k in 2:nrow(dataframe)) {dataframe[k, "dRotZ"] <- abs(dataframe[k, "rot_z"] - dataframe[k-1, "rot_z"])}
    for (k in 2:nrow(dataframe)) {ifelse(dataframe[k, "dX"] > 3 | dataframe[k, "dY"] > 3 | dataframe[k, "dZ"] > 3 | 
                                           dataframe[k, "dRotX"] > 3 | dataframe[k, "dRotY"] > 3 | dataframe[k, "dRotZ"] > 3, 
                                         dataframe[k, "excess"] <- 1, dataframe[k, "excess"] <- 0)}
    return(dataframe)}
  confound <- cal(confound)
  excess.sum <- sum(confound$excess, na.rm = T)
  id <- subject[i]
  ses <- session1[i]
  df <- data.frame(id, excess.sum)
  write.table(df, file = 
                paste0("/Shared/oleary/functional/UICL/BIDS/derivatives/fmriprep/rest_time2/fmriprep/excess_rest.txt"), 
              sep = '\t', row.names = F, col.names =! file.exists("/Shared/oleary/functional/UICL/BIDS/derivatives/fmriprep/rest_time2/fmriprep/excess_rest.txt"), quote = F, append = T)
}
