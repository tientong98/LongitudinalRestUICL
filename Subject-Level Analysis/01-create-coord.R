...
network$netName[network$netName == "VentralAttetion"] <- "VentralAttention"
...
van_5 <- network %>% 
  filter(netName == "VentralAttention") %>% 
  filter(radius == 5)%>% 
  select(c(4:6)) %>%
  mutate(value = 2) 
...
write.table(dmn_5, file = "dmn_5.txt", col.names = F, row.names = F, sep = " ", quote = F)
write.table(dmn_4, file = "dmn_4.txt", col.names = F, row.names = F, sep = " ", quote = F)
write.table(van_5, file = "van_5.txt", col.names = F, row.names = F, sep = " ", quote = F)
write.table(van_4, file = "van_4.txt", col.names = F, row.names = F, sep = " ", quote = F)
